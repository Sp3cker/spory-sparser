...
static void fprint_json_constant(FILE *f, const char *prefix, struct String s)
{
    fputc('"', f);
    if (!is_constant(s, prefix)) fprintf(f, "%s_", prefix);
    if (s.string_n > 0)
...
static void fprint_json_species(FILE *f, const char *prefix, struct String s)
{
    fputc('"', f);
    if (!is_constant(s, prefix)) fprintf(f, "%s_", prefix);
...
static void fprint_trainers_json(FILE *f, struct Parsed *parsed)
{
    ...
    for (int i = 0; i < parsed->trainers_n; i++)
    {
        ...
        {
            fprintf(f, "      \"class\": ");
            fprint_json_constant(f, "TRAINER_CLASS", trainer->class);
            fprintf(f, ",\n");
        }
        ...
        {
            fprintf(f, "      \"pic\": ");
            fprint_json_constant(f, "TRAINER_PIC", trainer->pic);
            fprintf(f, ",\n");
        }
        ...
        if (trainer->gender == GENDER_FEMALE)
            fprintf(f, "\"FEMALE\"");
        else if (trainer->gender == GENDER_MALE)
            fprintf(f, "\"MALE\"");
        else
            fprintf(f, "\"ANY\"");
        fprintf(f, ",\n");
        ...
        if (!is_empty_string(trainer->encounter_music))
        {
            ...
            fprint_json_constant(f, "TRAINER_ENCOUNTER_MUSIC", trainer->encounter_music);
            fprintf(f, ",\n");
        }
        ...
        if (trainer->items_n > 0)
        {
            ...
            for (int j = 0; j < trainer->items_n; j++)
            {
                if (j > 0)
                    fprintf(f, ", ");
                fprint_json_constant(f, "ITEM", trainer->items[j]);
            }
            fprintf(f, "],\n");
        }
        ...
        if (trainer->ai_flags_n > 0)
        {
            ...
            for (int j = 0; j < trainer->ai_flags_n; j++)
            {
                if (j > 0)
                    fprintf(f, ", ");
                fprint_json_constant(f, "AI_FLAG", trainer->ai_flags[j]);
            }
            fprintf(f, "],\n");
        }
        ...
        {
            fprintf(f, "      \"mugshot\": ");
            fprint_json_constant(f, "MUGSHOT_COLOR", trainer->mugshot);
            fprintf(f, ",\n");
        }
        ...
        if (!is_empty_string(trainer->starting_status))
        {
            ...
            fprint_json_constant(f, "STARTING_STATUS", trainer->starting_status);
            fprintf(f, ",\n");
        }
        ...
        {
            fprintf(f, "      \"poolRules\": ");
            fprint_json_constant(f, "POOL_RULESET", trainer->pool_rules);
            fprintf(f, ",\n");
        }
        ...
        {
            fprintf(f, "      \"poolPickFunctions\": ");
            fprint_json_constant(f, "POOL_PICK", trainer->pool_pick_functions);
            fprintf(f, ",\n");
        }
        ...
        {
            fprintf(f, "      \"poolPrune\": ");
            fprint_json_constant(f, "POOL_PRUNE", trainer->pool_prune);
            fprintf(f, ",\n");
        }
        ...
        {
            fprintf(f, "      \"copyPool\": ");
            fprint_json_string(f, trainer->copy_pool);
            fprintf(f, ",\n");
        }
        
        // Macro
        if (!is_empty_string(trainer->macro))
        {
            fprintf(f, "      \"macro\": ");
            fprint_json_string(f, trainer->macro);
            fprintf(f, ",\n");
        }
        ...
        {
            fprintf(f, "      \"backPic\": ");
            fprint_json_constant(f, "TRAINER_BACK_PIC", trainer->back_pic);
            fprintf(f, ",\n");
        }
        ...
        else
        {
            ...
            for (int j = 0; j < trainer->pokemon_n; j++)
            {
                ...
                // Species
                fprintf(f, "          \"species\": ");
                fprint_json_species(f, "SPECIES", pokemon->species);
                fprintf(f, ",\n");
                ...
                {
                    fprintf(f, "          \"heldItem\": ");
                    fprint_json_constant(f, "ITEM", pokemon->item);
                    fprintf(f, ",\n");
                }
                ...
                {
                    fprintf(f, "          \"ability\": ");
                    fprint_json_constant(f, "ABILITY", pokemon->ability);
                    fprintf(f, ",\n");
                }
                ...
                {
                    fprintf(f, "          \"ball\": ");
                    fprint_json_constant(f, "ITEM", pokemon->ball);
                    fprintf(f, ",\n");
                }
                ...
                {
                    fprintf(f, "          \"nature\": ");
                    fprint_json_constant(f, "NATURE", pokemon->nature);
                    fprintf(f, ",\n");
                }
                else
                {
                ...
                {
                    fprintf(f, "          \"teraType\": ");
                    fprint_json_constant(f, "TYPE", pokemon->tera_type);
                    fprintf(f, ",\n");
                }
                ...
                if (pokemon->tags_n > 0)
                {
                    ...
                    for (int k = 0; k < pokemon->tags_n; k++)
                    {
                        if (k > 0)
                            fprintf(f, ", ");
                        fprint_json_constant(f, "MON_POOL_TAG", pokemon->tags[k]);
                    }
                    fprintf(f, "],\n");
                }
                ...
                if (pokemon->moves_n > 0)
                {
                    ...
                    for (int k = 0; k < pokemon->moves_n; k++)
                    {
                        if (k > 0)
                            fprintf(f, ", ");
                        fprint_json_constant(f, "MOVE", pokemon->moves[k]);
                    }
                    fprintf(f, "]\n");
                }
                else
...
{
    fprintf(file, "Usage: %s [-j] -o <output> <source>\n", argv0);
    fprintf(file, "  -j         Output as JSON instead of C code\n");
    fprintf(file, "  -o <file>  Output file path\n");
    fprintf(file, "  -i <file>  Real source path for error messages\n");
}
